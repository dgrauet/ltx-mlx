...
Ported from ltx-trainer (Lightricks). This module provides a unified progress
display for training and validation sampling, encapsulating all Rich progress
bar logic in one place. Purely Python-native, no MLX-specific changes needed.
"""
...
class SamplingContext:
    ...
    Display format: ``Sampling X/Y [progress bar] step Z/W``
    """
...
class StandaloneSamplingProgress:
    ...
    Unlike ``SamplingContext`` (which integrates with ``TrainingProgress``),
    this class manages its own Rich ``Progress`` instance.
...
class TrainingProgress:
    ...
    Usage::
        ...
        with TrainingProgress(enabled=True, total_steps=1000) as progress:
            for step in range(1000):
                ...
                if should_validate:
                    ctx = progress.start_sampling(num_prompts=3, num_steps=30)
                    ...
                    ctx.cleanup()
    """
    ...
    def start_sampling(self, num_prompts: int, num_steps: int) -> SamplingContext:
        ...
        Returns:
            ``SamplingContext`` for tracking progress (no-op if disabled).
        """
