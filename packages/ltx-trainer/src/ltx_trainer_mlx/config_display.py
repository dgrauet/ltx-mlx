...
Ported from ltx-trainer (Lightricks). This module provides formatted console
output for ``LtxTrainerConfig``. Pure Python/Rich, no MLX-specific changes.
"""
...
def print_config(config: LtxTrainerConfig) -> None:
    ...
    def fmt(v: object, max_len: int = 55) -> str:
        ...
        if isinstance(v, bool):
            return "[green]yes[/]" if v else "[dim]no[/]"
    ...
    val = cfg.validation
    # accel section removed -- MLX has no mixed-precision / FSDP settings
    ...
    if hasattr(cfg.training_strategy, "generate_audio"):
        strategy_items.append(("Audio", fmt(cfg.training_strategy.generate_audio)))
